 ...
 * 高速フーリエ変換 (FFT) ライブラリ
 *
 * TRPGのダイス確率は「確率分布の畳み込み積分」で計算できますが、
 * 計算量が多いため、FFTを用いて計算を高速化しています。
 *
 * sumDistribution: 二つの分布を合成（加算）します。
 * subDistribution: 二つの分布を合成（減算）します。
 */
...
/**
 * 二つの確率変数の和の分布を返します。
 * @param {number[]} distribution1 - i番目の要素に確率変数1がiとなる確率を持つ長さ1024の配列
 * @param {number[]} distribution2 - i番目の要素に確率変数2がiとなる確率を持つ長さ1024の配列
 * @returns {number[]} i番目の要素に確率変数1+確率変数2がiとなる確率を持つ長さ1024の配列
 */
