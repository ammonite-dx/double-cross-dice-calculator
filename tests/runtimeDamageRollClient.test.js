...
import { createRuntimeDamageRollClient } from '../experiments/runtime-dr/client'
...
describe('runtime damage roll Worker client', () => {
  it('sends a transferable request and returns a defensive copy', async () => {
  ...
  it('rejects a Worker result whose probability total is invalid', async () => {
    const { client, workers } = createHarness()
  ...
  it('propagates calculation errors without discarding the Worker', async () => {
    const { client, workers } = createHarness()
    ...
    const next = client.calculate([0, 1], 4)
