...
describe('certified score statistics independent oracles', () => {
  it.each([0, 5, 10])(
    ...
    (target) => {
      ...
      expect(probability.upperBound - probability.lowerBound)
        .toBeLessThan(1e-6)
      expect(statistics.action.forcedFailureProbability.kind).toBe('exact')
      expect(statistics.action.forcedFailureProbability.value)
        .toBeCloseTo(0.1, 12)
    }
  )
  ...
  it('matches independent finite opposed enumeration and assigns ties to reaction', () => {
    ...
    expect(statistics.action.forcedFailureProbability.value)
      .toBeCloseTo(0.1, 12)
    expect(statistics.reaction.forcedFailureProbability.kind).toBe('exact')
    expect(statistics.reaction.forcedFailureProbability.value)
      .toBeCloseTo(0.1, 12)
  })
})
