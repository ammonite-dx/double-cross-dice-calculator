...
function validateRequest(weights, kazanari) {
  if (
  ...
  if (weights.length === 0 || weights.length > 203) {
    throw new RangeError('weights must contain 1 to 203 entries')
  }
  ...
  if (!Number.isInteger(kazanari) || kazanari < 0 || kazanari > 9) {
    throw new RangeError('kazanari must be an integer between 0 and 9')
  }
}
...
export function createRuntimeDamageRollClient({
  ...
  function calculate(weights, kazanari, { signal } = {}) {
    ...
    validateRequest(weights, kazanari)
    if (signal?.aborted) {
      return Promise.reject(createAbortError())
    }
