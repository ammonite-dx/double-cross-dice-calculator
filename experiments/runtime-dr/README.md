...
当時の統合プロトタイプは`kazanari = 0, 3, 9`について、正の攻撃力、負の攻撃力、防御ダイス、防御固定値、命中と非命中の混合を含む最終ダメージ分布を公開asset経路と比較し、最大絶対差$2\times10^{-6}$以内で一致することを確認しました。統合処理の現行実装は`src/calculation/DamageCalculator.js`と`src/runtime/CalculationClient.js`です。
