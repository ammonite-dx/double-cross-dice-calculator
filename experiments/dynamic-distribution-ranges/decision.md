...
| 判定・ダメージ作業分布 | ScoreはplannerのworkingLength、互換経路は2048要素 | 各作業配列の最後を作業範囲外を集約する末尾バケットとする。DRの直接Calculator/WorkerとDamageCalculatorは第2-Bで計画長へ対応し、公開結果は1024要素を維持する | [`src/calculation/ScoreCalculator.ts`](../../src/calculation/ScoreCalculator.ts)、[`src/calculation/DamageCalculator.ts`](../../src/calculation/DamageCalculator.ts)、[`src/calculation/RuntimeDamageRollCalculator.ts`](../../src/calculation/RuntimeDamageRollCalculator.ts) |
| DX入力 | dice 0–99、critical 2–11、shihai 0–19 | `shihai=0`は最大値の累積分布、`shihai>0`はダイス数方向DP。指定workingLengthの最後へ尾部を集約 | [`src/calculation/DxCalculator.ts`](../../src/calculation/DxCalculator.ts) |
| DX丸め | legacyは小数第6位、planner dynamicは未丸め | 公開JSONとplanなし経路は互換丸め、dynamic内部はtail 1e-8より粗い確率を消さない | [`src/calculation/DxCalculator.ts`](../../src/calculation/DxCalculator.ts) |
| 実行時DR入力 | damage dice 0–202、kazanari 0–9 | scoreの公開bucket 1023をdamage dice 202として扱うため、現行の最大値は202 | [`src/calculation/RuntimeDamageRollLimits.ts`](../../src/calculation/RuntimeDamageRollLimits.ts)、[`src/calculation/DamageCalculator.ts`](../../src/calculation/DamageCalculator.ts) |
| 実行時DR出力 | 既定2048要素、optionsで2以上の可変長 | `spectrumToDistribution`が`distributionLength - 1`以上を末尾へ加算し、長さ1はdamage 0とoverflowを分離できないため拒否 | [`src/calculation/RuntimeDamageRollCalculator.ts`](../../src/calculation/RuntimeDamageRollCalculator.ts)、[`src/calculation/RuntimeDamageRollLimits.ts`](../../src/calculation/RuntimeDamageRollLimits.ts) |
| 実行時DR FFT | 既定4096点、optionsで2の冪へ変更 | 必要supportより長いFFT長で周波数側を半分+1点評価し、残りを共役対称に補完 | [`src/calculation/RuntimeDamageRollCalculator.ts`](../../src/calculation/RuntimeDamageRollCalculator.ts) |
| 通常FFT畳み込み | 入力配列長`N`から`nextPowerOfTwo(2N-1)` | `sumDistribution`は出力長`N`へ戻し、`N-1`へ上限集約 | [`src/data/FFT.js`](../../src/data/FFT.js) |
| d10/livingdeadアセット | 1024要素、dice 0–223 | 1023以上はすでにアセット側で集約していた旧構成。現在のproductionはD10をruntime生成し、livingdeadはcanonical Backtrackでruntime生成する | [`src/calculation/D10Calculator.ts`](../../src/calculation/D10Calculator.ts)、[`src/data/ReferencePrecomputedDataRepository.js`](../../src/data/ReferencePrecomputedDataRepository.js) |
...
第5単位では、canonical damage envelope配列を独立加算する`sumCanonicalDamage`を追加した。入力は`modeledDistribution: true`とfinite/infiniteの`sourceSupport`を持つenvelopeに限定し、各resultを既存validatorで検証する。0件はdamage 0 identity、1件はFFTなしでresultを安全に再利用し、2件以上は明示valuesの完全線形畳み込みを行う。offset、modeled support、source supportはそれぞれsafe integer加算またはinfinite伝播とし、明示valuesが空のcomponentをtailのlowerBoundの一点値へ置き換えない。

overflowの計算はcomponent間の独立性を明示的に仮定する。nullはtail bound 0、exactはactual probability、upper-boundはprobability upper boundとして、`Math.log1p(-p)`と`-Math.expm1(sum)`で`1 - Π(1-p_i)`または`1 - Π(1-q_i)`を安定に計算し、p=1は1として扱う。全てnull/exactの場合はexact、upper-boundを含む場合は上記unionとFFT後の明示mass不足の大きい方をcap 1したupper-boundを出力する。mixed時のexact-only unionは`overflowProbabilityLowerBound`へ残し、potential tailを持つlowerBoundの最小値をoverflowへ設定する。probability 0でもerrorBoundを持つinert overflowはerror metadataを失わないようoutput objectまたはaggregate metadataへ保持する。

数値補正は、係数の`1e-12`までの微小負値だけを0へclampし、material negative、FFT convolution mass driftが`1e-8`を超える値、またはcanonical validatorが許容できない最終massはtyped numerical failureとする。source normalization drift（source explicit massとunion target、またはsource unionと最終explicit massの差）は失敗条件ではなく、`sourceMassDrift`と`aggregationErrorBound`へ記録する。exact outputはsource unionを`sourceOverflowProbability`へ保持し、最終explicit massから`1 - explicitMass`としてoutput probabilityを決める。空の明示valuesは許容誤差内のtarget massであっても空のままとし、source unionとの差をmetadataへ記録する。raw explicit mass、expected explicit mass、FFT drift、source errorBound合計をfreeze済みmetadataへ残し、丸め誤差を確率として黙って加算しない。
