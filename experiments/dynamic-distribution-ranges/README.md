...
このディレクトリは、入力範囲、DXの尾部打ち切り、ダメージの有限support、作業分布、FFT長、表示範囲、計算時間、メモリ量を一体で計画するための調査です。参照plannerのScore FFT契約は本番`RangePlanner`と同期しており、DR/Damage、配信JSON、UI入力制約は変更していません。
