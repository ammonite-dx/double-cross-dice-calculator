...
- 複数コンボの合計ダメージ分布の計算
- バックトラック後の侵蝕率分布の計算
- 《妖精の手》《絶対支配》《風鳴りの爪》、Dロイス《屍人》など、一部エフェクト・Dロイスの反映

ダイスロールの解釈、対応範囲、事前計算するダイス数の根拠は[`docs/dice-rules.md`](./docs/dice-rules.md)に記載しています。現在の実装では確率分布をインデックス0から1023の配列で表現します。`dx`、`d10`、`livingdead`は1023以上をインデックス1023へ集約し、`dr`は互換性のため1024以上を打ち切っています。
