...
full-tail Damageは、Score尾部の位置不明確率とDamage出力だけのoverflowを表示層でも区別する。canonical metadataの`projectionUncertainty.positionUnknownProbabilityUpperBound`はScore tail上限、Score tailの数値誤差、合成時の数値残差を加えて`[0,1]`へ切り上げた値であり、確率表示の半刻みである`DISPLAY_PROBABILITY_TOLERANCE = 5e-4`以下なら、PMFとupper-tailの表示値を変えない範囲の不確かさとしてprojectionを許可する。`outputOverflowLowerBound`が存在する場合はDamage出力overflowの下限を別に判定し、そのoverflowを位置不明tailの許容誤差へ紛れ込ませない。位置不明上限が許容誤差を超える場合、またはoutput overflowがwindowへ重なる場合は、値を推定せず`recalculate`または`not-projectable`を返す。upper-boundのDamage output overflowをupper-tailへ加算することはないが、exact output overflowでwindowより右側にある質量は従来どおりupper-tailへ加える。
