...
旧実装の`collapseDistribution`は、作業分布のインデックス0から1022を残し、インデックス1023以降の確率を公開分布の最後へ合計する互換処理でした。このsymbolと自己検証は、canonical経路や公開assetの再生成から参照されていないことを確認して後続の互換surface監査で削除しました。現在は`DistributionResult`の`toPublishedBucketDistribution`が、overflow metadataを検査したうえで必要な場合だけ同じpublished-bucket形状へ明示的に投影します。上限外の確率を安全条件なしに捨てたり、一点へ読み替えたりしないことが重要です。
...
過去の`getNightmareSingleResult`には、100～119%の区間の終端と71～99%の区間の始端が連続せず、最終侵蝕率がちょうど100%になる確率をどの区分にも加えない不整合がありました。`9571f08`で区間を連続させ、境界値テストを追加済みです。現在の実装では、100%の結果は100～119%区分へ分類されます。
...
`CalculationClient`はdefault dependencyとして`planCalculationRanges`を持ち、`planCheck(params, difficulty?, policy?)`、`planAttackCombo(params, policy?)`、`planBacktrack(params, policy?)`からsnapshot済み入力の計画を直接取得できます。計算APIはcanonicalの`calculateCheckCanonical`、`calculateAttackCanonical`/`calculateAttackCanonicalBatch`、`calculateCanonicalTotalDamage`、`calculateBacktrackCanonical`に統一され、各APIが計算前に同じ計画を生成します。`options.rangePolicy`と`options.onRangePlan`はcanonical APIへ渡され、callbackは計画直後に1回だけ同期通知されます。`accepted`が`false`の場合は`CalculationRangeError`を`plan`と`rejectionReasons`付きでthrowし、アセット読込、DX計算、DR Worker起動、結果生成を開始しません。legacy CalculationClient APIとroute preloadはPhase 7 cleanup第2単位で削除しました。
...
backtrackの計算では、`calculateBacktrackCanonical`が同じpreflightの`plan.backtrack`をcanonical計算coreへ渡します。`runtime options`（`signal`、`requestId`など）と計画を別引数に分け、`workingMax`、`workingLength`、`fftLength`を計算側で再検証します。計画ありでは完全supportをオンデマンド生成し、legacy client wrapperやroute preloadへfallbackしません。
...
Phase 7第1実装単位では、`createBacktrackCanonicalRunner`をcanonical専用runnerとして接続し、Backtrackの初期計算・入力再計算を`calculateBacktrackCanonical`から`createBacktrackCanonicalPresentation`へ一本化しました。`InputPanel.vue`と`Backtrack.vue`から一時`canonicalOptIn` toggleとlegacy分岐を削除し、初期計算も`onMounted`から同じlatest-wins runnerへ渡します。canonical adapterのpresentation error、ResourceGuard rejection、range rejection、abort、stale result、disposeでは旧結果へfallbackせず結果をclearし、retryで再度commitできます。route preloadとCalculationClient legacy APIの削除は第2実装単位で完了し、legacy core/assets/comparison fixtureは維持しています。

Phase 7のAttack実装単位では、`Attack.vue`の初期計算、validated input、combo操作を`createAttackCanonicalRunner`の一つのlatest-wins laneへ統合し、`calculateAttackCanonicalBatch`から`createAttackCanonicalDisplayPresentation`を通るcanonical batch/presentationだけをproduction chart、summary、totalへ渡します。legacy初期計算、combo runner、total runner、productionの`canonicalOptIn`/debug panel、legacy fallbackは接続から除き、canonicalのrange/resource/generic/presentation/asset errorでは結果をclearして同じrunnerでretryします。Scoreのunsupported expected valueを`—`とする契約を維持し、Damage/Totalの非保証値も`—`とし、通常の不確かさwarningはUIへ出しません。route preloadとCalculationClient legacy APIはcleanup第2単位で削除し、canonical防御D10のlazy asset、既存`RuntimeDamageRollWorker`、legacy core/assets、1024/1022境界、任意display windowは維持しています。
...
- Phase 7 legacy cleanup第2単位では、`CalculationClient`のlegacy計算API、legacy score/damage/backtrack dependency、fallback、route `prepare`と全計算routeのpreload guardを削除しました。client-level legacy比較fixtureと専用client/prepareテストを削除・canonical契約へ移植し、下位legacy core/wrappers、comparison/migration/rule/asset tests、JSON/generatorはPhase 8まで維持しています。
...
## R25-C Runtime Damage Worker preemption (current)

The mixed damage-roll primitive remains the only calculation executed in a Worker. `RuntimeDamageRollClient` owns a main-thread queue and posts at most one job at a time. Identical requests share a job through independent subscribers; an Abort removes only the caller's subscriber. If the last subscriber leaves an active job, the client terminates that Worker, settles the job with `AbortError`, and starts the next queued job on a fresh Worker. If the last subscriber leaves a queued job, it is removed without starting a Worker. Worker identity guards ignore late messages and errors from terminated Workers, while returned calculation errors remain local to their job and fatal transport errors reject pending jobs and require a later Worker recreation.

`CalculationRequestCoordinator` signals the active request when a newer request supersedes it and removes older queued work. A snapshot or resource-preflight failure also aborts the stale active request and discards an older queued item before publishing the failure state. Execute calls remain serialized: the latest request starts only after the superseded request settles. `CalculationClient` holds one ResourceGuard lease per request and releases it in `finally`; no underlying-work tracking or `onUnderlyingSettled` callback is part of the current contract. The Worker message protocol and numerical damage-roll kernel are unchanged. See [`ADR 0004`](./adr/0004-runtime-damage-worker-preemption.md) for the decision record.
...
full-tail Damageは、Score尾部の位置不明確率とDamage出力だけのoverflowを表示層でも区別する。canonical metadataの`projectionUncertainty.positionUnknownProbabilityUpperBound`はScore tail上限、Score tailの数値誤差、合成時の数値残差を加えて`[0,1]`へ切り上げた値であり、確率表示の半刻みである`DISPLAY_PROBABILITY_TOLERANCE = 5e-4`以下なら、位置不明確率の誤差をUI表示精度内に抑えられるため、PMFとupper-tailへのprojectionを許可する。この閾値は丸め結果が常に完全一致することを主張するものではない。`outputOverflowLowerBound`が存在する場合はDamage出力overflowの下限を別に判定し、そのoverflowを位置不明tailの許容誤差へ紛れ込ませない。位置不明上限が許容誤差を超える場合、またはoutput overflowがwindowへ重なる場合は、値を推定せず`recalculate`または`not-projectable`を返す。upper-boundのDamage output overflowをupper-tailへ加算することはないが、exact output overflowでwindowより右側にある質量は従来どおりupper-tailへ加える。
...
このadapterは確率値を丸めず、既存UIの確率パーセント1桁丸めやsummary丸めも担当しない。現行`DistributionResult`はoverflowのlowerBoundを「そこ以上にある下限」として保持するが、overflow massがexplicit coverage内へどう分配されたかを証明しないため、output overflowのoverlapを拒否するのが現契約の安全境界である。Score tailについてはproducerが明示した`projectionUncertainty`と表示精度の許容誤差を検証し、許容範囲内だけをprojectionする。producerが「overflowはexplicitMaxより上」と強化できれば、adapter側の拒否範囲をさらに狭められるが、既存producerを壊す契約変更は後続の親判断へ残す。production ChartSetterと各producerへの接続はこの単位の対象外である。
