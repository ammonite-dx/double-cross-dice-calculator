...
- BacktrackとAttackのcanonical default化はPhase 7の実装単位として完了した。full-tail Attackのresource planning・cost model校正・Chrome desktop/CPU 4x受入は`8c7d10c`で完了し、action-only damage range assertionを`569c278`で整合させた。production warning/hard thresholdは50/200msを暫定維持する。2026-08-25のローカルCI相当（Node、data check/generator verify、JavaScript/Markdown/Python lint、Node/generator tests、simulation、build、diff check）は成功したが、PR未作成のためGitHub PR CIは未実行である。Phase 7全体のlegacy計算・fallback削除、表示範囲拡張、JSON整理は未完了である。1024は事前計算・固定長配列由来の比較用上限であり、legacy/published-bucket比較境界であってcanonical schemaや最終production表示の上限とはしない。
- AttackのScore/Damage表示範囲は999上限を撤廃し、任意の非負safe integerを受け付ける。`0..100`、`0..999`、`0..1000`、`0..1023`、`0..1024`、`0..1200`、`1000..1200`、`0..20000`の入力・coverage・resource判定を回帰テストで固定し、単一点`min === max`も有効とした。表示点数・メモリ・計算量のresource plannerによるrejectは維持する。Runtime DRのrange/FFTとfull-tail Damage rangeもplannerから動的に導出し、202Dをproduction semantic capとして扱わない。実UIで同じwindowを操作する最終受入は未実施である。
- 2026-08-25のCodex In-app Browser受入では、Score/Damageの`0..1200`、`1000..1200`、Scoreの`0..20000` resource reject、`0..100`への復旧、通常・99D/critical=2・《妖精の手》単独・《支配の領域》単独を確認した。成功時はcanvas 2・alert/dialog 0・console warn/error 0、reject時はScoreを抑止してDamageを保持し、復旧後に2 canvasへ戻った。full-tail overflowの数値誤差boundは`e43ff0b`で明示prefix終端+1を保持するよう修正した。Long Taskと>202Dの実UI入力は未測定・未実施で、前者は別の性能検証、後者は現行UI入力上限整理後の受入とする。
...
- 現在の状態: BacktrackとAttackのcanonical default化、CalculationClient legacy API/route preload cleanup、full-tail Attack resource計測、cost model校正、Chrome desktop/CPU 4x受入、production threshold 50/200msの暫定維持判断は完了した。Phase 7全体は未完了であり、legacy core/wrapper・assets/JSON/generatorの整理、任意表示範囲の最終受入、低速実機・Firefox/WebKit・実UI描画を含むthreshold再評価は後続作業とする。
