...
- 完了（R10 Runtime Validation Simplification、2026-09-04）: trusted internal ordinary object境界から過剰なreflection・prototype traversal・descriptor防御を除去した。production実装は`ce743f07f7e656b79b5569a3d506726caae20bf0`／`89170d090068c29c4982a8aaccf0dd5dc18cfe1a`、Final Closure Follow-upのテスト契約縮小は`e4e53568856c65c27b31ad76b3cc7f25b988f6e9`である。accessor／getter、custom prototype、symbols、Proxy、reflection failureの挙動は未規定とし、受入・拒否・読み取り回数を保証しない。4対象モジュールのforbidden reflection pattern 0件、保護領域の差分0件、fresh full gate、production smoke、P0／P1／P2=0を確認した。詳細は[`refactoring-runtime-validation.md`](./refactoring-runtime-validation.md)を参照する。R11命名整理は完了し、次はR12 core分割とする。
- 完了（R11 Canonical / Migration Terminology Cleanup、2026-09-05）: production唯一経路になった計算API、型、feature state、runtime、presentation、calculationからmigration-onlyな`Canonical`接頭辞と同一値の移行用aliasを除去した。`published-bucket` projectionとadapter固有の`legacy` terminologyは現在も意味を持つcompatibility boundaryとして保持した。実装は`269d20e63cdf8eb319341d2c3a0ea28435c4f85b`、命名境界テストは`6e36594b557ba63d1c67b3eda61f5c2241bd5126`である。sourceのCanonical filename、廃止production identifier、compatibility re-export shimは0件で、protected areaの差分も0件だった。Vitest 74 files / 866 tests、generator 18、simulation 13、data 32 assets、typecheck／ESLint／Ruff／Markdown lint、runtime DX 20,000、build 408 modules、production smokeをGREENで確認し、P0／P1／P2=0、R11を`CLOSED / GREEN`とした。詳細は[`r11-terminology-cleanup.md`](./r11-terminology-cleanup.md)を参照する。次はR12 core module decompositionとする。
- 完了（R12 Core Module Decomposition、2026-09-05）: 共通DX tail certificateを`src/calculation/DxTailModel.js`へ集約し、Score、Damage、Backtrackの範囲計画を`src/calculation/planning/`の操作別moduleへ分離した。safe arithmetic・FFT見積りは`PlanningMath`、policy・display正規化は`RangePolicy`、資源見積り・warning／rejectは`ResourcePlan`へ移し、`RangePlanner.js`は後方互換のorchestration facadeとして維持した。`DxCalculator`と`ScoreCalculator`から`RangePlanner`への依存をなくし、tail APIのRangePlanner再exportも廃止した。working range、tail certificate、resource threshold、DistributionResult、Worker protocol、UI、公開データ、generatorの意味は変更していない。`51c7ae5`（DX tail抽出）と`dab6bd4`（planning責務分割）で実装し、`tests/dxTailModel.test.js`と`tests/corePlanningArchitecture.test.js`を追加した。Vitest 76 files / 873 tests、typecheck、ESLint、`git diff --check`をGREENで確認した。詳細は[`refactoring-core-decomposition.md`](./refactoring-core-decomposition.md)を参照する。R12を`CLOSED / GREEN`とし、次はR13の候補整理へ進む。
...
- 完了（R7 final closure follow-up、2026-09-03、最終実装 `3c1dccd`）: Attack Damage表示の`0..20000` resource rejectionと`0..100` recoveryをproduction browser smokeで確認し、reject時canvas 0・alert、recovery時canonical result再commit・canvas 2・Summary「合計」行を固定した。コンボ削除後はDOM削除だけでなくcanvas data差分をcanonical commitの条件に追加し、表示要素数の変化も`waitForResultCommit`で検出する。production sourceの追加変更はなく、full gateはVitest 69 files／857 tests、data 32 assets、generator／simulation、Ruff、typecheck、ESLint、Markdown lint 32 files／0 issues、runtime DX 20,000 cases、build、production smoke、`git diff --check`のすべてGREEN、P0／P1／P2は0件である。詳細は[`refactoring-attack-feature.md`](./refactoring-attack-feature.md)を参照し、R7を最終的に`CLOSED / GREEN`、次をR8とする。
