...
- 現在の状態: BacktrackとAttackのcanonical default化、CalculationClient legacy API/route preload cleanup、full-tail Attack resource計測、cost model校正、Chrome desktop/CPU 4x受入、production threshold 50/200msの暫定維持判断、主要Attack表示windowのin-app Chromium受入は完了した。Phase 7全体は未完了であり、legacy core/wrapper・assets/JSON/generatorの整理、低速実機・Firefox/WebKit・実UI描画を含むthreshold再評価は後続作業とする。
