...
- 完了（R4 Check featureization、2026-09-03、HEAD `09eec97`）: Check固有のsnapshot、presentation、controller、display orchestration、UIを`src/features/check`へ移し、`src/views/Check.vue`を薄いroute adapterにした。`useCheck`はCalculationClientを注入し、初期canonical計算、validated input、latest-wins、Abort、表示windowのREUSE／RECALCULATE／RESOURCE_REJECTED／NOT_PROJECTABLE、disposeを維持する。旧application・components path、広い`checkData` prop、旧importは退役し、構造テストとfull gateで依存境界を固定した。詳細は[`refactoring-check-feature.md`](./refactoring-check-feature.md)を参照する。次はR5でCheckとAttackの入力validation・form contract共通化を検討する。
