...
8. 完了（Phase 8-2A）: Attackのlegacy chart adapterを`LegacyChartSetter.js`へ分離し、canonical adapterを`ChartSetter.js`へ集約した。`ChartPercentages.js`の表示丸めとgolden testを追加し、公開asset、JSON、generator、計算意味論は変更していない。次はD10 lazy fetchのdependency contractを固定したうえで、PrecomputedDataRepositoryのproduction/reference分離を検討する
