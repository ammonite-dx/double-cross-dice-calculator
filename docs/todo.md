...
- 完了（Phase 8-2G7〜2G10）: legacy comparison/migration依存を独立oracleへ移行し、legacy calculation surface、dense JSON、schema-v1、旧JS generator、実行不能なlegacy harnessを撤去した。残存するpublished-bucket、LegacyChartSetter、rounding alias、Reference repository等はconsumerと保持理由をinventoryへ記録し、Phase 8をclosureした。
