...
10. 完了（R8 `src/data`責務分離、2026-09-03）: Distribution／FFTを`src/core/probability/`、ChartPaletteを`src/shared/theme/`、schema／Reference repositoryを`tooling/reference-data/`へ移し、旧`src/data`と互換re-exportを削除した。全consumer、ESLint architecture rule、責務境界テストを更新し、Distribution／FFT、schema-v2/revision-1、reference cache、paletteの契約を維持した。実装コミットは`ddb43ad`、`f625e02`、`924c817`で、詳細は[`phase8-inventory.md`](./phase8-inventory.md)と[`refactoring-architecture-boundaries.md`](./refactoring-architecture-boundaries.md)に記録する。次はR9のapplication／presentation／runtime責務再評価とする。
