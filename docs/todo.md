...
8. 進行中（R23 User-supervised UI / UX Review、R23-A実施完了）: `d001bc7`でproduct-owner観察を記録し、`aa7fa7d`で公開版とのsemantic parity診断と異なる入力のmulti-combo captureを追加した。`fbf323e`でDamage／Total Damageのstable bounded期待値表示を共通formatterへ統一し、`48c374d`で13 fixtureの精度監査（exact 1、lower-bound 12、bounded 0、丸め境界crossing 0）を実施した。`6f10a14`でvisual parity、Backtrack responsive layout、footer、6／8／9px labelのexperiment-only prototypeを作成した。productionへ採用したのは表示formatterだけで、UI見た目、計算core、tail、overflow、Worker、公開assetは変更していない。R23は`IN REVIEW`、UI-04A〜C・UI-06・UI-07・UI-01・UI-05はproduct-owner visual review待ち、広い近似表示は監査済み・別途判断待ちとする。詳細は[`r23-user-supervised-ui-review.md`](./r23-user-supervised-ui-review.md)を参照する。
