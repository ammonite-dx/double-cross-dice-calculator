...
- 部分完了: `d30b3d1`でfull-tail overflowの位置契約を修正し、Score尾部由来の位置不明massは`lowerBound=0`、Damage出力だけの右側overflowは最終出力境界をlower boundとするよう分離した。修正後のCodex In-app Browserでは、通常およびaction `99D/critical=2`の`Damage 0..100`が`not-projectable`となり、alert 1・canvas 0・console warn/error 0を確認した。以前のcanvas 2受入は修正前の挙動であり、今回の安全契約を満たす最終受入とは分けて扱う。Long Taskはこの受入では測定していない。
- 確認済み: 現行Attack入力フォームの上限とは独立に、production `CalculationClient.planAttackCombo()`はaction `dice=99`・`critical=2`から`scoreValueUpperBound=2271`、`maxDamageDice=228`、`rawSupportMax=2280`、`workingLength=1024`、`fftLength=4096`、`accepted=true`、拒否理由なしを導出した。`maxDamageDice > 202`のRangePlan計画は確認済みであるが、Score/Damage chart ready、任意window matrixの最終browser受入は位置不明tailのprojectability設計が未解決のため未完了とする。
