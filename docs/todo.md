...
- 次段階（Phase 8-2G5）: `src/data/PrecomputedDataRepository.js`と`tests/precomputedDataRepository.test.js`を対象に、compatibility facadeの単独削除を判断する。Phase 8-2G全体は完了扱いにしない。
...
- 状態: Phase 7のバックトラックとAttackのcanonical default化、ブラウザ受入、legacy削除前の最終比較、CalculationClient/route cleanup第2単位、任意表示範囲の最終受入、最終HEAD gateは完了した。Phase 8-1 inventoryとPhase 8-2A/2B/2C/2D/2E/2F/2G1/2G2/2G3/2G4（ChartSetter split、CalculationClient dependency contract、production browser smoke、precomputed repository source split、reference/legacy importer audit、runtimeRuleValidation actual migration、facade importer migration、calculation barrel importer migration・削除）も完了している。legacy core/wrapper・assets/生成物/JSON/generator、compatibility facadeの削除、benchmark/experimentの個別cleanupは後続作業とする。
