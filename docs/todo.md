...
8. 完了（Phase 8-2A/2B）: Attackのlegacy chart adapterを`LegacyChartSetter.js`へ分離し、canonical adapterを`ChartSetter.js`へ集約した。`ChartPercentages.js`の表示丸めとgolden test、`tests/productionDependencyContract.test.js`のCalculationClient dependency contractを追加した。公開asset、JSON、generator、計算意味論、production codeはPhase 8-2Bでは変更していない。次はbrowser network smokeを固定し、その後にPrecomputedDataRepositoryのproduction/reference分離を検討する
