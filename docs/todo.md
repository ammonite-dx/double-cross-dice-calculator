...
- 将来拡張TODO: R23-C1Bで通常DXの負の`skill`と`shihai`の保守的tail first-moment certificateは実装済み。未対応の無限supportでは、action側`yousei`（exact-youseiのfirst-moment residual）を引き続き検討する。Total Damageへの証明書伝播、published-bucket cleanup、実験資料の整理も後続課題である。canonical既定化、debug panel/toggle削除、legacy計算・fallback削除はPhase 7で完了した。
