...
## R20 Graph-first UIと描画投影（完了）

- 開始点: `0ce4f54`（R19キャッシュ中立フォローアップ完了）。計算意味論、Worker境界、ResourceGuard、R17のincremental execution、R21対象のpublished-bucket互換と旧batch APIは変更しない。
- 目的: 論理表示範囲とChart.jsの描画点数を分離し、PMFの区間bin集約、upper-tailの正確な代表threshold、レスポンシブな描画予算、Backtrackの横棒グラフとHTML確率表を導入する。
- 契約: graph-firstの画面順序、PMF質量保存、upper-tail単調性、overflowと不確かさの意味、論理範囲のsafe integer・scan/resource安全性、resize時の計算再実行禁止を維持する。詳細は[`r20-chart-rendering.md`](./r20-chart-rendering.md)を参照する。
- 実装順序: 契約文書、Chart.js非依存projectionとplanner責務分離、Check／Attack移行、Backtrack再設計、ブラウザ密度測定、最終文書化の順で進める。
- 判断保留: `0..20000`を描画分離後に受理するかは、計算・論理scan resourceの実測後に決める。Firefox／WebKitはR19と同じ環境制約がある場合、追加導入をclosure必須条件にしない。
- 完了: `ProbabilityChartProjection`とレスポンシブな`ProbabilityRenderBudget`を追加し、Check／AttackのPMF bin集約・upper-tail threshold投影をproduction表示へ接続した。resize時は計算を再実行せず、既存displayからChart.jsデータだけを再投影する。Backtrackは横棒グラフとHTML確率表へ移行し、6pxのdatalabel依存を削除した。
- 検証: R20専用のChrome描画実験で100、1,000、4,096、16,384、20,000論理座標を比較し、投影点数を最大512へ抑えた。通常条件とCPU 4x条件でdense描画の50ms超Long Taskと投影後の描画時間を記録した。詳細は[`r20-chart-rendering.md`](./r20-chart-rendering.md)と[`experiments/r20-chart-rendering/`](../experiments/r20-chart-rendering/)を参照する。
- 判断: `0..20000`を無条件に受理する入力・計算policyは保留し、`DisplayRangePlanner`の計算・scan資源制限を維持する。Chart.js側には旧1024点上限を設けず、コンテナ幅から導出した描画予算で投影する。計算意味論、Worker境界、ResourceGuard、published-bucket互換、旧batch API、API／MCP目標は変更していない。
