...
- 完了（R5 Shared Input Validation、2026-09-03、実装最終 `df7ffdc`）: `src/shared/validation`にVue非依存のIntegerRules、ScoreInputRules、DisplayRangeRules、LatestValidationGateを追加し、Check／AttackのScore・表示範囲validationと全フォームのasync latest-wins gateを移行した。InputDomainを数値domainの正本として再利用し、yousei／shihai同時指定、負のskill、critical 2..11、single-point range、`0..20000`のform-valid／resource-rejected契約を維持した。構造テストでshared layerの依存方向、重複rule、旧gate exportを固定し、Node 22.23.2、Vitest 65 files / 826 tests、runtime DX 20,000 cases、data 32 assets、generator、simulation、Ruff、typecheck、ESLint、Markdown lint（29 files / 0 issues）、build、production browser smokeをGREENで確認した。詳細は[`refactoring-shared-validation.md`](./refactoring-shared-validation.md)に記録する。次はR6またはR7の設計判断とする。
