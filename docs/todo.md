...
- 未完了: Phase 7全体のlegacy core/wrapper・生成物/JSON整理。Vue完全mountは既存Node test環境制約により未実施で、runner/router behavior testと今回のin-app Chromium受入で補完している。低速実機、Firefox/WebKit、実UI描画を含むthreshold再評価は後続とする。
