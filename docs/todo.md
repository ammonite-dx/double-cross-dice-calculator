...
- 状態: Phase 7のバックトラックとAttackのcanonical default化、ブラウザ受入、legacy削除前の最終比較、CalculationClient/route cleanup第2単位、任意表示範囲の最終受入、最終HEAD gateは完了した。Phase 8-1 inventoryとPhase 8-2A/2B/2C/2D（ChartSetter split、CalculationClient dependency contract、production browser smoke、precomputed repository source split）も完了している。legacy core/wrapper・assets/生成物/JSON/generatorの整理は後続作業とする。
