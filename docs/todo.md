...
- 完了（R4 Check featureization、2026-09-03、HEAD `09eec97`）: Checkのsnapshot・presentationを`src/features/check/model`へ移し、計算と表示のstate machineを`useCheck.ts`へ抽出した。Check UI 10ファイルを`src/features/check/ui`へ移動し、`CheckPage.vue`と薄い`src/views/Check.vue`を導入した。初期canonical計算、validated snapshot、latest-wins、Abort、表示windowのpreflight・再利用・再計算・resource rejection、disposeと既存のprops/eventsを維持した。構造テストで旧path、広い`checkData` prop、model→UI逆依存を禁止し、Node 22.23.2、Vitest 62 files / 810 tests、runtime DX 20,000 cases、data 32 assets、generator、simulation、Ruff、typecheck、ESLint、Markdown lint、build、production browser smokeをGREENで確認した。詳細は[`refactoring-check-feature.md`](./refactoring-check-feature.md)に記録する。次はR5としてCheckとAttackの入力validation・form contract共通化を検討する。
