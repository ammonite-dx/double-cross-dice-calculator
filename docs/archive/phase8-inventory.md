...
| `src/calculation/D10Calculator.js` | runtime D10 primitive、`calculateD10Distribution`、provider | `production` | `CalculationClient`、Damage、Backtrack | D10 runtime、canonical、range tests | ブラウザ内完全support生成、bounded provider cache | generator d10 output/manifestは比較用 | runtime/asset equivalence、browser smoke | `keep` | D10を公開assetへ戻さない | JS tests、browser smoke |
...
R8の実装コミットは、`ddb43ad`（probability primitives）、`f625e02`（chart palette）、`924c817`（reference data supportと責務境界テスト）、`64ccf0d`（core／theme ESLint境界とlintText回帰テスト）、`9afdd93`（theme相対sibling境界のESLint／source scan／lintText回帰テスト）である。旧`src/data`ディレクトリ、5旧path、互換re-export shimは削除し、旧importはproduction source・testsから移行した。`tests/dataResponsibilitiesArchitecture.test.js`は新pathの存在、旧pathの不在、旧importの再導入禁止、production→reference逆依存禁止、probability/theme境界、themeから`../validation`／`../chart`への相対sibling境界を恒久検査する。
...
R8の最終implementation gateは2026-09-04、実装SHA `9afdd935c469a37fea4bbe9586e4732c7012ba9f`で成功した。Node.jsは22.23.2、`data:check`／`data:verify-generator`は各32 assets、Vitestは71 files / 865 tests、generator testは18 passed / 13 deselected、simulationは13 passed / 18 deselected、Ruff、typecheck、ESLint、Markdown lint（33 files / 0 issues）、`npm run verify:runtime-dx`（20,000 cases、full enumeration 32194.1886 ms、max absolute difference約`1.0000000000287557e-6`、tolerance `0.000001000001`、総和誤差約`1.55e-15`、non-finite 0、negative 0）、production build（408 modules transformed）、production browser smoke、`git diff --check`が成功した。smokeではCheck／Attack／Backtrackの100D再計算、表示範囲reject/recovery、canvas描画、precomputed/D10 request 0、console warning/error 0、pageerror 0、same-origin HTTP error 0、same-origin request failure 0を確認した。P0／P1／P2は0件であり、R8は`CLOSED / GREEN`である。詳細は[`refactoring-data-responsibilities.md`](./refactoring-data-responsibilities.md)を参照する。R9のapplication/presentation再配置、API/MCP、algorithm変更はR8に含めない。
