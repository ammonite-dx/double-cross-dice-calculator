...
- advanced settings checkbox自身へ`label="高度な設定"`を設定し、SFC source inspection testを廃止した。model/controller invariantはVitestで、accessible checkboxとOFF→ONのhidden value resetはproduction browser smokeで検証する。
...
- `npm run verify:all`: 成功。Vitestは104 files / 1096 tests、typecheck・ESLint・Markdown lint（84 files / 0 issues）・build（454 modules）・diff checkが成功した。
- production browser smokeはCheck・Attack・Backtrackの全経路で成功し、role/nameによる「高度な設定」checkbox取得、OFF→ONのhidden value reset、precomputed requests 0件、AttackのD10 requests 0件、browser diagnostics 0件を確認した。
