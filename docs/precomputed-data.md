...
`d10`と`livingdead`の疎な分布は、計算で必要になったものだけを長さ1024の配列へ展開します。
`dr`は読み込んだ`kazanari`ごとに、ダメージ計算の走査順に合わせた型付き配列のビューを一度だけ
構築します。データ取得・検証・これらのキャッシュは`PrecomputedDataRepository.js`に集約します。
